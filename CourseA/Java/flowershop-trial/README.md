# Open Fast Path トライアルトレーニング 
-ー　 version 2020.06.10　ー

[PDF版](https://github.com/ofp-training-admin/flowershop-trial/blob/master/OpenFastPathTraialTraiing.pdf)
...
## トライアルプログラムのダウンロード（全員実施）

### トライアル用のソースコードのダウンロードと反映

```
cd ~/workspace
git clone https://github.com/ofp-training-admin/flowershop-trial.git
oft-training-admin
openfastpath0001
```
...
```
cd ~/workspace/oldshop-demo/repository
code .
```

3. VS CodeでSQLToolのセットアップ（環境構築で実施済みの場合スキップ）
「アクティブバー」より「SQLTool」を選ぶ（まだSQLTool）を入手していない場合は、別紙「Open Fast Pathトレーニング環境構築手順」を参照。

	* 「アクティブバー」から「拡張機能」を選択し、検索窓に”sqltool”と入力。
	* 検索結果からSQLToolsを選択しインストール。
	* 「アクティブバー」から「拡張機能」を選択し、検索窓に”sqltool”と入力し、「アクティブバー」から「SQLTools MySQL/Maria DB」を選択肢インストール。
	* 「アクティブバー」からSQLTool を選択。「CONNECTIONS」から「Add New Connection」のアイコンをクリックしMySQLを選択。
	* データベースの選択でMySQLを選び下記項目を設定。
		...
		* Username                    user
		* Password                      password
		* Save To Global Setting  チェックなし
	* 「TEST CONNECTION」を実行して「SUCCESS…」と表示されたら「SAVE CONNECTION」をクリック。
	* 「CONNECTIONS」に追加された「vs_con_flower_db」をクリックし接続されることを確認。
	
4. 販売管理テーブルの生成
	* 「アクティブバー」から「エクスプローラ」を選択し、ファイルより`create_tables.sql`を右クリックして「Attach connection to this file」を選択。`SHIFT+CTRL+P`で「コマンドパレット」を開き、「SQLTools:Connection Run This File」を選び、接続先DBに`vs_con_flower_db `を選択して実行。
...
3. .稼働確認
	...
	* ブラウザにて`http://localhost:8090/`でトップページを表示し、ログイン画面→メニュー画面→販売一覧画面に遷移しデータが表示されることを確認。 
...
1. Gitより デモアプリケーションをダウンロード
```
cd ~/workspace
git clone https://github.com/ofp-training-admin/oldshop-demo.git
oft-training-admin
openfastpath0001
```
...
wget http://archive.apache.org/dist/tomcat/tomcat-9/v9.0.34/bin/apache-tomcat-9.0.34.tar.gz
...
4. Tomcat管理Webの管理者ユーザを追加
 `/usr/local/tomcat/conf/tomcat-user.xml`の最終行`</tomcat-users>`の前に下記を追加する
```
